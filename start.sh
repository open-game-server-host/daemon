...
if [ ! -f "$DOCKER_SOCK_PATH" ]; then
    printf "WARN  Docker socket not found! ($DOCKER_SOCK_PATH)\n"
fi
DOCKER_SOCK_PATH="/var/run/docker.sock"
if [ ! -S "$DOCKER_SOCK_PATH" ]; then
    printf "ERROR Docker socket not found! ($DOCKER_SOCK_PATH)\n"
    exit 1
else
    printf "INFO  Using docker socket at '$DOCKER_SOCK_PATH' instead\n"
fi
...
docker run -d -u $(id -u "$USER"):$(getent group docker | cut -d: -f3) --read-only --cpus=1 --memory=500m -v $API_KEY_PATH:/ogsh/apikey -v $DOCKER_SOCK_PATH:/var/run/docker.sock -v "$CONTAINER_FILES_PATH":/ogsh/container_files -v "$APP_ARCHIVES_PATH":/ogsh/app_archives -v "$STARTUP_FILES_PATH":/ogsh/startup_files --name $CONTAINER_NAME $CONTAINER_TAG
